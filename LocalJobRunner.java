...
class LocalJobRunner implements JobSubmissionProtocol {
  ...
  private class Job extends Thread
    ...
    public void showData(Path mapout, JobConf localConf)
    {
      try {
        ...
        LOG.info(codec);
        //Merger.Segment<Text, Text> seg = new Merger.Segment<Text, Text>(conf, fs, mapOut, codec, false); 
    ...
    public void run() {
      ...
      try {
        ...
        for (int j = 0; j < taskSplitMetaInfos.length; j++) {
            int i = j ;
            LOG.info(i);
          if (!this.isInterrupted()) {
            ...
            showData(mapOutput.getOutputFile(), localConf);
